alter table public.chat_user_preferences
  add column if not exists focused_context_enabled boolean not null default false;
...
alter table public.chat_usage_records add constraint chat_usage_records_request_kind_check check (
  request_kind in ('chat', 'title', 'reasoning_summary', 'image_analysis', 'image_text_analysis', 'image_visual_analysis', 'image_followup', 'chat_summary', 'chat_recall', 'dreaming', 'todo_planner', 'deep_research', 'automation', 'context_router')
);
