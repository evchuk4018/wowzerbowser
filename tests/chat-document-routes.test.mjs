...
test("delete route removes the document through the authenticated store dependency",async()=>{const deleted=[];const handler=createDeleteHandler({authorizeOwnerSession:async(request)=>{assert.ok(request instanceof Request);return {id:"owner"};},deleteDocument:async(input)=>deleted.push(input)});const response=await handler(new Request("http://test",{method:"DELETE",headers:{cookie:"wowzerbowser.session-token=session","content-type":"application/json"},body:JSON.stringify({conversationId:"conversation",documentId:"document",contentType:"application/pdf"})}));assert.equal(response.status,200);assert.deepEqual(await response.json(),{deleted:true});assert.deepEqual(deleted,[{ownerId:"owner",conversationId:"conversation",documentId:"document",contentType:"application/pdf"}]);});
