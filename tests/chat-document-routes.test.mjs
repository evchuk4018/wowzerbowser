...
test("finalize route passes the signed PDF URL to ingestion without returning or logging it",async()=>{
 const logs=[];
 ...
 try {
  ...
  const handler=createFinalizeHandler({
   ...
   createSignedDocumentDownloadUrl:async(input)=>{signedInput=input;return signedUrl;},
  ...
  assert.deepEqual(signedInput,{ownerId:"owner",conversationId:"conversation",documentId:"document",contentType:"application/pdf"});
  assert.equal(ingestInput.downloadUrl,signedUrl);
  assert.doesNotMatch(JSON.stringify(await response.json()),/secret-download-token/);
  assert.doesNotMatch(JSON.stringify(logs),/secret-download-token|access-token/);
 } finally { console.info=original; }
});

test("finalize route redacts invalid signed download URL failures",async()=>{
 const logs=[];
 const original=console.info;
 const secretUrl="https://storage.test/storage/v1/object/sign/chat-documents/wrong/conversation/document.pdf?token=secret-download-token";
 console.info=(entry)=>logs.push(entry);
 try {
  const handler=createFinalizeHandler({
   authorizeOwnerSession:async()=>({id:"owner"}),
   getServerClient:()=>({storage:{from:()=>({download:async()=>({data:new Blob(["%PDF-raw"]),error:null})})}}),
   createSignedDocumentDownloadUrl:async()=>{throw new ChatDocumentError("document_storage_invalid_url",`Invalid URL: ${secretUrl}`,502);},
   ingestPdf:async()=>{throw Error("not called");},
   ingestDocx:async()=>{throw Error("not called");},
  });
  const response=await handler(new Request("http://test",{method:"POST",headers:{authorization:"Bearer access-token", "content-type":"application/json"},body:JSON.stringify({conversationId:"conversation",documentId:"document",userMessageId:"message",jobId:"job",filename:"a.pdf",contentType:"application/pdf"})}));
  const payload=await response.json();
  assert.equal(response.status,502);
  assert.equal(payload.failedStage,"supabase-download");
  assert.equal(payload.error,"The document could not be downloaded.");
