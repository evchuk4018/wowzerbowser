...
test("keeps mobile history drawer movement progressive and chat centered", async () => {
  ...
  assert.match(page, /onLostPointerCapture=\{cancelDrawerGesture\}/);
  assert.match(page, /!event\.isPrimary/);
  assert.match(page, /drawerGestureRef\.current\s*\)\s*return/);
  assert.match(page, /const reasoningSummary = target\.closest<HTMLElement>\("\.reasoning-summary"\)/);
  assert.match(page, /const captureTarget = reasoningSummary \?\? event\.currentTarget/);
  assert.match(page, /captureTarget,/);
  assert.match(page, /summaryTarget: reasoningSummary/);
  assert.match(page, /suppressReasoningClickRef\.current = summaryTarget/);
  assert.match(page, /window\.setTimeout\(\(\) => \{[\s\S]*?suppressReasoningClickRef\.current === summaryTarget[\s\S]*?suppressReasoningClickRef\.current = null/);
  assert.match(page, /onClickCapture=\{handleDrawerClickCapture\}/);
  assert.match(page, /event\.stopPropagation\(\)/);
  assert.match(page, /captureTarget\.setPointerCapture\(event\.pointerId\)/);
  assert.match(page, /gesture\.axis !== "horizontal"\) \{[\s\S]*?abandonDrawerGesture\(event\)/);
  assert.match(page, /\.assistant-markdown pre/);
  assert.match(page, /\.assistant-markdown table/);
  ...
  assert.match(styles, /\.transcript[\s\S]*?touch-action: pan-y;/);
  assert.match(styles, /\.reasoning-content[\s\S]*?touch-action: pan-y;/);
  assert.match(styles, /\.assistant-markdown pre[\s\S]*?touch-action: pan-x pan-y;/);
  assert.match(styles, /\.assistant-markdown table[\s\S]*?touch-action: pan-x pan-y;/);
  assert.match(styles, /\.assistant-markdown \.katex-display[\s\S]*?touch-action: pan-x pan-y;/);
