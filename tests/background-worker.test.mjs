...
test("worker loop enforces one active chat and document task and aborts on shutdown", async () => {
  let chatClaims = 0;
  ...
  await new Promise((resolve) => { chatStarted = resolve; documentStarted = resolve; });
...
test("worker source uses durable queue claims and the web routes do not execute providers", async () => {
  const [migration, worker, route, finalize] = await Promise.all([
