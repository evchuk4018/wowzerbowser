...
export function AddMcpServerModal({ onClose, onAdded }: { onClose: () => void; onAdded: (connector: ConnectorCatalogItem) => void }) {
  const [draft, setDraft] = useState({ name: "", description: "", endpointUrl: "", token: "" });
  const [error, setError] = useState(""); const [busy, setBusy] = useState(false);
  async function save() { setBusy(true); setError(""); try { onAdded(await addMcpServer(draft)); onClose(); } catch (reason) { setError(reason instanceof Error ? reason.message : "MCP server could not be added."); } finally { setBusy(false); } }
