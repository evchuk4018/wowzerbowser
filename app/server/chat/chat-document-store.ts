...
import { ChatDocumentError, PDF_PAGE_EXTRACTION_METHODS, type ChatDocumentAttachment, type ChatDocumentPage, type ChatDocumentPageFailure } from "../../../lib/chat-document";
  ...
  const register = async () => {
    ...
    await withChatPersistenceRetry(async () => {
      await query(
        ...
        [databaseOwnerId(input.ownerId), input.conversationId, jsonb(input.document), input.userMessageId, input.jobId, jsonb(input.pages), documentStoragePath(object.objectKey)],
      );
...
export async function getDocumentPages(ownerId: string, conversationId: string, pdfId: string, start = 1, end = 100000): Promise<ChatDocumentPage[]> {
  ...
  return data.map((page) => ({ pageNumber: Number(page.page_number), text: page.text, extractionMethod: storedPageMethod(page.extraction_method, page.text), ...(storedPageFailure(page.failure) ? { failure: storedPageFailure(page.failure) } : {}) }));
}
...
export async function deleteChatDocumentsForConversation(ownerId: string, conversationId: string): Promise<void> {
  const rows = await query<{ storage_object_id: string | null }>("select storage_object_id from chat_documents where owner_id=$1 and conversation_id=$2", [databaseOwnerId(ownerId), conversationId]);
