...
export async function executePdfTool(call: ChatToolCall, context: { ownerId: string; conversationId: string; allowedPdfIds: ReadonlySet<string> }): Promise<ChatToolResult> {
  let args: Record<string, unknown>; try { args = JSON.parse(call.arguments); } catch { return fail(call, "Invalid PDF tool arguments."); }
  const pdfId = typeof args.pdfId === "string" ? args.pdfId : "";
  if (!context.allowedPdfIds.has(pdfId) || !(await getAuthorizedDocument(context.ownerId, context.conversationId, pdfId))) return fail(call, "PDF is not authorized for this conversation.");
  ...
  if (call.name === READ_PDF_PAGES_TOOL_NAME) {
    ...
    if (!document || Number(end) > document.pageCount) return fail(call, "The requested page range is outside this PDF.");
    const pages = await getDocumentPages(context.ownerId, context.conversationId, pdfId, Number(start), Number(end));
    return { id: call.id, name: call.name, ok: true, stdout: pages.map((p) => `[PDF page ${p.pageNumber}]\n${p.text}`).join("\n\n"), stderr: "" };
  }
