...
export const WORKSPACE_TOOL_DEFINITIONS: ModelToolDefinition[] = [
  ...
  { type: "function", function: { name: WORKSPACE_SEARCH_TOOL_NAME, description: "Search filenames and text contents in the persistent conversation workspace and return matching paths and line excerpts.", parameters: { type: "object", additionalProperties: false, required: ["query"], properties: { query: { type: "string", minLength: 1, maxLength: WORKSPACE_LIMITS.maxSearchQueryLength }, path: { ...path }, maxResults: { type: "integer", minimum: 1, maximum: WORKSPACE_LIMITS.maxSearchResults } } } } },
...
export function availableWorkspaceTools(enabled: boolean): ModelToolDefinition[] {
  return enabled ? WORKSPACE_TOOL_DEFINITIONS : [];
}
