...
export function workspaceToolDefinitions(): ModelToolDefinition[] {
  ...
  return [
    ...
    { type: "function", function: { name: WORKSPACE_SEARCH_TOOL_NAME, description: `Search filenames and text contents in the persistent conversation workspace and return up to ${defaultResults} matches by default.`, parameters: { type: "object", additionalProperties: false, required: ["query"], properties: { query: { type: "string", minLength: 1, maxLength: WORKSPACE_LIMITS.maxSearchQueryLength }, path: { ...path }, maxResults: { type: "integer", minimum: 1, maximum: WORKSPACE_LIMITS.maxSearchResults, default: defaultResults } } } } },
