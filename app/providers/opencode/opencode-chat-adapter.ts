...
import { OpenCodeError } from "./opencode-catalog-adapter";
...
export function buildOpenCodeMessages(request: ChatRequest, options: ChatProviderRoundOptions): ProviderMessage[] {
...
export async function* streamOpenCodeChatRound(request: ChatRequest, metadata: ChatModelInfo, options: ChatProviderRoundOptions, signal: AbortSignal, fetchImpl: typeof fetch = fetch): AsyncGenerator<ChatStreamEvent> {
  if (!openCodeApiKey()) throw new OpenCodeError("OpenCode is not configured.", 503);
  ...
  const response = await fetchImpl(`${OPENCODE_BASE_URL}/chat/completions`, {
    method: "POST", headers: openCodeHeaders(), signal,
    body: JSON.stringify({
      model: request.model.model, messages: buildOpenCodeMessages(request, options), stream: true,
      ...(reasoning ? { reasoning } : {}),
  ...
  if (!response.ok || !response.body) {
    ...
    throw new OpenCodeError(detail.slice(0, 240) || `OpenCode request failed (${response.status}).`, response.status >= 500 ? 502 : response.status);
  }
  ...
  const consume = function* (block: string): Generator<ChatStreamEvent> {
    ...
    if (chunk.error) throw new OpenCodeError(typeof chunk.error.message === "string" ? chunk.error.message : "OpenCode streaming error.");
    ...
    if (measured) yield { type: "done", usage: measured, provider: "opencode", model: typeof chunk.model === "string" ? chunk.model : request.model.model, ...(typeof chunk.usage?.cost === "number" ? { exactCostUsd: chunk.usage.cost } : {}), pricing: metadata.pricing };
  };
  try {
    while (true) {
    ...
    for (const call of [...calls.values()]) {
      if (!call.id || !call.name) throw new OpenCodeError("OpenCode returned an incomplete tool call.");
...
export const openCodeChatProviderAdapter: ChatProviderAdapter = {
  provider: "opencode",
  assertConfigured() { if (!openCodeApiKey()) throw new OpenCodeError("OpenCode is not configured.", 503); },
  streamRound: streamOpenCodeChatRound,
};
