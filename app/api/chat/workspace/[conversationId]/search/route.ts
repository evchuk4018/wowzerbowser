...
export async function POST(request: Request, context: { params: Promise<{ conversationId: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ matches: await searchWorkspaceFiles(owner.id, conversationId, body.query, typeof body.path === "string" ? body.path : "", typeof body.maxResults === "number" ? body.maxResults : 50) });
  } catch (error) {
